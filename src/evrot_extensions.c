...
static PyObject *gradU(PyObject *self, PyObject *args) {
    // arguments
    PyArrayObject *theta, *ik, *jk, *V;
    int k;

    //locals
    int i, j;                                   // array indexing
    double tt, sin_tt, cos_tt;                  // angles
    double *p_theta, *p_ik, *p_jk;              // pointers to 1D arrays
    double **p_V;                               // pointers to 2D array
    ...
    if (ik->nd != 1 || ik->descr->type_num != NPY_INT64) {
        PyErr_SetString(PyExc_ValueError,
        "ik must be a one-dimensional integer array");
        return NULL;
    }
    if (jk->nd != 1 || jk->descr->type_num != NPY_INT64) {
        PyErr_SetString(PyExc_ValueError,
        "jk must be a one-dimensional float array");
        return NULL;
    }
    if (V->nd != 2 || V->descr->type_num != NPY_DOUBLE) {
        PyErr_SetString(PyExc_ValueError,
        "V must be a two-dimensional float array");
        return NULL;
    }

    // set pointers
    p_V = pymatrix_to_Carrayptrs(V); // remember to free this memory!
    p_theta = pyvector_to_Carrayptrs(theta);
    p_ik = pyvector_to_Carrayptrs(ik);
    p_jk = pyvector_to_Carrayptrs(jk);
...
static PyObject *build_Uab(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "O!iiO!O!i",
                          &PyArray_Type, &theta, &a, &b, &PyArray_Type, &ik, &PyArray_Type, &jk, &dim)) {
        return NULL;
    }
    // type check on input
    if (theta->nd != 1 || theta->descr->type_num != NPY_DOUBLE) {
...
#else
static struct PyModuleDef evrot_extensions_module = {
   PyModuleDef_HEAD_INIT,
   "evrot_extensions",   /* name of module */
   NULL, /* module documentation, may be NULL */
   -1,       /* size of per-interpreter state of the module,
                or -1 if the module keeps state in global variables. */
   evrot_extensions_methods
};

PyMODINIT_FUNC
PyInit_evrot_extensions(void)
{
    import_array();
    return PyModule_Create(&evrot_extensions_module);
}
#endif
